...
def collect_subject_rows(subject_dirs, mapping):
    ...
    for sub_dir in sorted(subject_dirs):
        ...
        for code in orientation_codes:
            key = str(code)
            if key not in alpha_map or key not in gamma_map:
                continue
            orientation_rows.append(
                {
                    "code": int(code),
                    "deg": float(mapping["orientation_code_degrees"][key]),
                    "alpha": float(alpha_map[key][pick_idx]),
                    "gamma": float(gamma_map[key][pick_idx]),
                }
            )
